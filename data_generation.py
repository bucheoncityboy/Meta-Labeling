...
#dual_regime(or single)이 만든 'raw'의 데이터를 받아, 2차 모델(메타 모델)을 학습시키는 데 필요한 **'요리 재료'**로 가공하는 역할을 합니다.
def prep_data(data, with_flags):
    # Set target variable
    data['target'] = data['rets'].apply(lambda x: 0 if x < 0 else 1).shift(-1)  # Binary classification 0 or 1

    # Create data set
    data['target_rets'] = data['rets'].shift(-1)  # Add target rets for debugging 실제 수
    data.dropna(inplace=True)

    # Auto-correlation trading rule: trade sign of previous day.
    data['pmodel'] = data['rets'].shift(1).apply(lambda x: 1 if x > 0.0 else 0)
